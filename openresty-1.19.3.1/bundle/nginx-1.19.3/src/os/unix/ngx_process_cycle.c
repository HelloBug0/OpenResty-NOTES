...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    ngx_start_worker_processes(cycle, ccf->worker_processes,
                               NGX_PROCESS_RESPAWN);
    ngx_start_cache_manager_processes(cycle, 0); /* 处理逻辑和 ngx_start_worker_processes 相似，只是只创建一个进程 */
    ngx_start_privileged_agent_processes(cycle, 0); /* 处理逻辑和 ngx_start_worker_processes 相似，只是只创建一个进程 */
    ...
    for ( ;; ) {
        ...
        sigsuspend(&set); /* master进程挂起，等待信号产生唤醒 */
        ...
        if (ngx_reap) {
            ...
            live = ngx_reap_children(cycle); /* work进程的善后工作，以及work进程的重启等工作 */
        }
...
ngx_start_worker_processes(ngx_cycle_t *cycle, ngx_int_t n, ngx_int_t type)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        ngx_spawn_process(cycle, ngx_worker_process_cycle, /* 进程创建之后的执行函数 ngx_worker_process_cycle */
        ...
        ngx_pass_open_channel(cycle, &ch); /* 向已经创建的work进程传递刚刚创建的work进程信息 */
    }
}
...
ngx_pass_open_channel(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    ...
    for (i = 0; i < ngx_last_process; i++) {
        ...
        ngx_write_channel(ngx_processes[i].channel[0], /* 将进程信息传递给已经建立好的进程 */

...
main(int argc, char *const *argv)
{
    ...
    /* 初始化日志文件名称和打开的日志文件描述符 */
    log = ngx_log_init(ngx_prefix); // ngx_prefix 工作路径 全局变量，调用函数之前指针为空
    if (log == NULL) {
        return 1;
    }
    ...
    init_cycle.log = log; /* 日志指针，保存日志相关的信息 */
    ngx_cycle = &init_cycle; /* init_cycle是栈变量 */
    ...
    ngx_os_status(cycle->log); /* 日志输出系统信息 */
    ...
    if (!ngx_inherited && ccf->daemon) {
        if (ngx_daemon(cycle->log) != NGX_OK) { /* 函数正常结束之后，父进程退出，子进程执行以下程序 */
            return 1;
        }
    ...
    if (ngx_create_pidfile(&ccf->pid, cycle->log) != NGX_OK) { /* 创建并初始化nginx.pid文件 */
        return 1;
    }
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    return ngx_set_inherited_sockets(cycle); /* 初始化需要监听的socket信息 */
}

...
struct ngx_http_request_s {
    ...
    ngx_pool_t                       *pool;
    ngx_buf_t                        *header_in; /* 接收HTTP请求的内存区，主要用于接收http头部 */

    ngx_http_headers_in_t             headers_in; /* 接收后的HTTP请求头部格式化后的数据 */
    ngx_http_headers_out_t            headers_out; /* 将 要发送的HTTP头部保存在该结构中，http框架会将其序列化发送出去 */

    ngx_http_request_body_t          *request_body; /* 接收请求包体的数据结构 */

    time_t                            lingering_time;
    time_t                            start_sec; /* 当前请求初始化时的时间 */
    ngx_msec_t                        start_msec;

    ngx_uint_t                        method; /* ngx_http_process_request_line函数接收到http请求行之后解析出的信息 */
    ...
    ngx_http_posted_request_t        *posted_requests; /* 所有的子请求都通过这个链表连接 */
    ...
    unsigned                          pipeline:1;
    unsigned                          chunked:1;
    unsigned                          header_only:1; /* 发送的响应中只有响应头 */

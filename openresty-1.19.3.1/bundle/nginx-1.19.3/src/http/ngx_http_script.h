...
typedef struct { /* 用于编译解释 */
    ngx_conf_t                 *cf;
    ngx_str_t                  *source;

    ngx_array_t               **flushes; /* *flushes是一个数组，数组元素是变量的下标（变量指配置文件中配置的变量，如$host） */
    ngx_array_t               **lengths; /* *lengths是一个数组，数组元素是一个结构体，结构体有两个字段，一个是获取变量长度的函数，一个是变量在ngx_conf_t中的变量下标 */
    ngx_array_t               **values; /* *valuess是一个数组，数组元素是一个结构体，结构体有两个字段，一个是获取变量取值的函数，一个是变量在ngx_conf_t中的变量下标 */
...
typedef struct {
    ngx_str_t                   value;
    ngx_uint_t                 *flushes; /* 记录变量索引 */
    void                       *lengths; /* 计算变量长度的函数指针 */
    void                       *values; /* 计算变量的函数指针 */
...
typedef struct {
    ngx_http_script_code_pt     code; /* 获得变量值或者变量长度的处理函数 */
    uintptr_t                   index; /* 变量在全局变量ngx_conf_t cf中的变量下标 */

...
ngx_http_script_variables_count(ngx_str_t *value)
{
    ...
    for (n = 0, i = 0; i < value->len; i++) {
        if (value->data[i] == '$') { /* 判断字符串中$的个数，即变量的个数 */
            n++;
        }
    }
...
ngx_http_script_compile(ngx_http_script_compile_t *sc)
{
    ...
    for (i = 0; i < sc->source->len; /* void */ ) {
        ...
        if (sc->source->data[i] == '?' && sc->compile_args) {
            ...
            if (ngx_http_script_add_args_code(sc) != NGX_OK) { /* 该函数处理和 ngx_http_script_add_var_code 逻辑相似，可以看出sc->length和sc->values里保存的数据的长度并不相同 */
                return NGX_ERROR;
            }
        ...
        if (ngx_http_script_add_copy_code(sc, &name, (i == sc->source->len)) /* 非变量部分，拷贝保存处理 */
            != NGX_OK)
        {
            return NGX_ERROR;
        }
...
ngx_http_script_add_code(ngx_array_t *codes, size_t size, void *code)
{
    ...
    if (code) { /* code是传出参数，如果code非空，code里取值+（新的数组-旧的数组的指针）*/
        if (elts != codes->elts) { /* 数组发生扩容，造成数组首地址发生变化 */
            p = code;
...
ngx_http_script_add_var_code(ngx_http_script_compile_t *sc, ngx_str_t *name)
{
    ...
    code = ngx_http_script_add_code(*sc->lengths, /* 创建数组的时候，指定的元素的大小是1，这里的第二个参数就是在数组里申请一个元素地址的时候指定元素的大小 */
                                    sizeof(ngx_http_script_var_code_t), NULL); /* 获得数组中的一个元素位置 */
    if (code == NULL) {
        return NGX_ERROR;
    }

    code->code = (ngx_http_script_code_pt) (void *)
                                             ngx_http_script_copy_var_len_code; /* 获得变量长度的处理函数 */
    ...
    code->code = ngx_http_script_copy_var_code; /* 获得变量值的处理函数 */
    code->index = (uintptr_t) index;
